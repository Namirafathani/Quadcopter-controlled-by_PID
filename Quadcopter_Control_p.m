...
if param.traj == 2
    P_var.Kp = 6.2; %P_var.Ki = 15;
    P_var.Kp_z = 0.9; %P_var.Ki_z = 1;
    P_var.Kp_xy = 0.105;
elseif param.traj == 1
    P_var.Kp = 9.2; %P_var.Ki = 1.5;
    P_var.Kp_z = 0.16; %P_var.Ki_z = 6;
    P_var.Kp_xy = 0.10;
end

if param.traj == 3
    P_var.Kp = 6.0; %P_var.Ki = 1.5;
    P_var.Kp_z = 0.13; %P_var.Ki_z = 6;
    P_var.Kp_xy = 0.19;
elseif param.traj == 4
    P_var.Kp = 10.0; %P_var.Ki = 15;
    P_var.Kp_z = 4; %P_var.Ki_z = 1;
    P_var.Kp_xy = 0.04;
end
...
for idx_t = 1:n_sim
    ...
    control_input = PI(omega_I_current, eta_current, xi_current, nu_I_current, param, P_var, xi_des, xi_des_d, xi_des_dd); % omega square
...
function ctrl = PI(omega_I, eta, xi, nu_I, param, PI_var, xi_des, xi_des_d, xi_des_dd)
    ...
    T = (param.g + PI_var.Kp_z * (z_des - z)) * (param.m / (cos(phi) * cos(theta)));
    tau_phi = (PI_var.Kp * (phi_des - phi)) * param.I(1,1);
    tau_theta = (PI_var.Kp * (theta_des - theta)) * param.I(2,2);
    tau_psi = (PI_var.Kp * (psi_des - psi)) * param.I(3,3);
...
function eta_des = pos2ang(xi, xi_des, xi_des_d, xi_des_dd, nu_I, param, PI_var)
    ...
    eta_des(1) = -(param.m * y_des_dd + param.Kd(2, 2) * y_des_d + PI_var.Kp_xy * (y_des - y)) / T_des;
    eta_des(2) = (param.m * x_des_dd + param.Kd(1, 1) *  x_des_d  + PI_var.Kp_xy * (x_des - x)) / T_des;
end